-- Orders
	...
	BEGIN
		...
		SELECT *,products.name as name,
			(SELECT json_agg(json_build_object('order_product_option_id', opo.order_product_option_id, 'product_option_id' , opo.product_option_id, 'product_option_value_id' , opo.product_option_value_id, 'option', opo.option, 'name' , opo.name, 'price' , opo.price, 'type' , opo.type) )
	...
	BEGIN
		...
		@EACH(:products) 
			INSERT INTO order_product 
				( order_id, @KEYS(:each) )
			VALUES ( @result.order, :each ) RETURNING order_product_id;
		
		-- insert product option
		@EACH(:product_options) 
			INSERT INTO order_product_option 
				( order_id, @KEYS(:each) )
			VALUES ( @result.order, :each ) RETURNING order_product_option_id;
		
		-- insert order products
		@EACH(:totals) 
			INSERT INTO order_total 
				( order_id, @KEYS(:each) )
			VALUES ( @result.order, :each ) RETURNING order_total_id;
	...
	BEGIN
		...
		VALUES ( :order_id, :product ) RETURNING order_product_id;
	
		@EACH(:product_options) 
			INSERT INTO order_product_option 
				( order_id, order_product_id, @KEYS(:each) )
			VALUES ( :order_id, @result.order_product, :each ) RETURNING order_product_option_id;
	...
	BEGIN
		...
		@EACH(:product_options) 
			...
			ON CONFLICT ("order_id", "order_product_id") DO UPDATE SET SET @LIST(:each);
    END
	...
	BEGIN
		...
		INSERT INTO order_total 
			( order_id, @KEYS(:total) )
		VALUES ( :order_id, :total ) RETURNING order_total_id;
